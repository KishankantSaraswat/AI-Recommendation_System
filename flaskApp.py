...
app = Flask(__name__)
app.secret_key = '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'
...
def format_user_prompt(question):
    ...
    Please provide the answer in numbered points. Each point should begin with a number (1., 2., 3., etc.), and the total response should not exceed 30 words.
    """
...
def index():
    if request.method == 'POST':
        ...
        return render_template('index.html', response=ai_response, chat_history=chat_history)
    
    return render_template('index.html', response=None, chat_history=session.get('chat_history', []))
